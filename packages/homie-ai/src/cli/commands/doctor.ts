...
export async function runDoctorCommand(
  ...
  if (loaded) {
    ...
    const agentKey = env[HOMIE_AGENT_KEY_ENV]?.trim() ?? '';
    if (!agentKey) {
      issues.push(`wallet: missing ${HOMIE_AGENT_KEY_ENV}`);
    } else if (!isValidAgentRuntimePrivateKey(agentKey)) {
      issues.push(`wallet: invalid ${HOMIE_AGENT_KEY_ENV} format (expected 0x + 64 hex chars)`);
    } else {
