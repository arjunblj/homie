...
interface ToolEnv extends NodeJS.ProcessEnv {
  HOMIE_PIPER_BIN?: string | undefined;
  HOMIE_PIPER_MODEL?: string | undefined;
  HOMIE_FFMPEG_BIN?: string | undefined;
}
...
export const createPiperTtsSynthesizer = (
  ...
  return {
    async synthesizeVoiceNote(text, opts) {
      ...
      const modelPath = e.HOMIE_PIPER_MODEL?.trim() ?? '';
      if (!modelPath) return { ok: false, error: 'not_enabled: set HOMIE_PIPER_MODEL' };

      const piperName = (e.HOMIE_PIPER_BIN?.trim() || 'piper').trim();
      const piperBin = resolveBin(piperName);
      if (!piperBin) return { ok: false, error: `not_enabled: "${piperName}" not found in PATH` };

      const ffmpegName = (e.HOMIE_FFMPEG_BIN?.trim() || 'ffmpeg').trim();
