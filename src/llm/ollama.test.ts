...
describe('resolveOllamaBaseUrl', () => {
  test('returns null for invalid URLs', () => {
    const prev = process.env.OPENHOMIE_OLLAMA_URL;
    try {
      process.env.OPENHOMIE_OLLAMA_URL = 'not a url';
      expect(resolveOllamaBaseUrl()).toBeNull();
    } finally {
      if (prev === undefined) delete process.env.OPENHOMIE_OLLAMA_URL;
      else process.env.OPENHOMIE_OLLAMA_URL = prev;
    }
  });
  ...
  test('returns null for non-http(s) schemes', () => {
    const prev = process.env.OPENHOMIE_OLLAMA_URL;
    try {
      process.env.OPENHOMIE_OLLAMA_URL = 'file:///tmp/ollama';
      expect(resolveOllamaBaseUrl()).toBeNull();
    } finally {
      if (prev === undefined) delete process.env.OPENHOMIE_OLLAMA_URL;
      else process.env.OPENHOMIE_OLLAMA_URL = prev;
  ...
  test('defaults to localhost-only unless requireLocalhost=false', () => {
    const prev = process.env.OPENHOMIE_OLLAMA_URL;
    try {
      process.env.OPENHOMIE_OLLAMA_URL = 'http://example.com:11434/';
      expect(resolveOllamaBaseUrl()).toBeNull();
  ...
  test('strips trailing slashes', () => {
    const prev = process.env.OPENHOMIE_OLLAMA_URL;
    try {
      process.env.OPENHOMIE_OLLAMA_URL = 'http://127.0.0.1:11434///';
