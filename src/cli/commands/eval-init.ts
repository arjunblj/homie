...
import type { OpenhomieConfig, OpenhomieProvider } from '../../config/types.js';
...
interface BackendEntry {
  ...
  label: string;
  create: () => LLMBackend;
  providerKind: OpenhomieProvider['kind'];
}
...
const buildEvalConfig = (projectDir: string, provider: OpenhomieProvider): OpenhomieConfig => {
...
const runCasesForBackend = async (opts: {
  ...
  const provider = { kind: opts.entry.providerKind } as OpenhomieProvider;
...
export async function runEvalInitCommand(opts: GlobalOpts, cmdArgs: string[]): Promise<void> {
  ...
  const projectDir = path.join(tmpdir(), `openhomie-eval-init-${Date.now()}`);
