...
interface DeployEnv extends NodeJS.ProcessEnv {
  ...
  OPENHOMIE_DEPLOY_REPO?: string;
  OPENHOMIE_DEPLOY_REF?: string;
  OPENHOMIE_DEPLOY_MAX_PER_REQUEST_USD?: string;
  OPENHOMIE_DEPLOY_MAX_PER_DAY_USD?: string;
}
...
const DEFAULT_RUNTIME_IMAGE_TAG = 'openhomie-runtime:latest';
const DEFAULT_RUNTIME_REPO = 'https://github.com/arjunblj/openhomie.git';
...
const REMOTE_RUNTIME_DIR = '/opt/openhomie';
const REMOTE_RUNTIME_USER = 'openhomie';
...
const buildComposeYaml = (runtimeImageTag: string, includeSignalApi: boolean): string => {
  const lines = [
    ...
    '    env_file: .env',
    '    environment:',
    '      OPENHOMIE_CONFIG_PATH: /app/homie.toml',
    '    volumes:',
...
const copyRuntimeBundle = async (input: {
  ...
  const composePath = path.join(os.tmpdir(), `openhomie-compose-${Date.now().toString(36)}.yml`);
...
const verifyRemoteHealth = async (input: {
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const result = await sshExec({
      ...
      expectedHostKeyPins: input.hostKeyPins,
      command:
        "cd /opt/openhomie && (wget -qO- 'http://127.0.0.1:9091/health' || curl -fsS 'http://127.0.0.1:9091/health')",
      timeoutMs: 8_000,
    });
...
export async function runDeployCommand(
  ...
  const runtimeRef = assertSingleLineValue(
    'OPENHOMIE_DEPLOY_REF',
    runtimeEnv.OPENHOMIE_DEPLOY_REF?.trim() || DEFAULT_RUNTIME_REF,
  );
  ...
  const deployMaxPerRequestUsd = resolvePositiveUsdLimit(
    runtimeEnv.OPENHOMIE_DEPLOY_MAX_PER_REQUEST_USD,
    DEFAULT_DEPLOY_MAX_PER_REQUEST_USD,
    'OPENHOMIE_DEPLOY_MAX_PER_REQUEST_USD',
  );
  const deployMaxPerDayUsd = resolvePositiveUsdLimit(
    runtimeEnv.OPENHOMIE_DEPLOY_MAX_PER_DAY_USD,
  ...
  try {
    ...
    if (shouldRunPhaseFrom(startPhase, 'provision')) {
      ...
      const name = normalizeDropletName(
        state.droplet?.name ?? parsed.name ?? (generatedName || 'openhomie-vps'),
      );
      ...
      const keysDir = path.join(os.homedir(), '.openhomie', 'deploy-keys', normalizedProjectSlug);
      const keyPair = await generateSshKeyPair(keysDir, 'id_ed25519_openhomie');
      ...
      if (!keyId) {
        ...
        try {
          const accountKey = await mppDo.createSshKey(
            `openhomie-${Date.now().toString(36).slice(-8)}`,
            keyPair.publicKey,
          );
          keyId = accountKey.id;
      ...
      if (!ready) {
        ...
        const regionCandidates = [
          region,
          ...(parsed.region || env.OPENHOMIE_DEPLOY_REGION
            ? []
            : allRegions
    ...
    const ensureActiveHost = async (): Promise<string> => {
      ...
      state = {
        ...
        droplet: {
          id: ready.id,
          name: state.droplet?.name ?? `openhomie-${String(ready.id)}`,
