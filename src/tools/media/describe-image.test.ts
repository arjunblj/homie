...
describe('media tools: describe_image', () => {
  ...
  test('returns not-enabled message when vision model is unset', async () => {
    const prev = process.env.OPENHOMIE_OLLAMA_VISION_MODEL;
    try {
      delete process.env.OPENHOMIE_OLLAMA_VISION_MODEL;
      ...
      if (prev === undefined) delete process.env.OPENHOMIE_OLLAMA_VISION_MODEL;
      else process.env.OPENHOMIE_OLLAMA_VISION_MODEL = prev;
    }
  });
});
