...
describe('media tools: transcribe_audio', () => {
  test('returns not-enabled message when OPENHOMIE_WHISPER_MODEL is unset', async () => {
    const prev = process.env.OPENHOMIE_WHISPER_MODEL;
    try {
      delete process.env.OPENHOMIE_WHISPER_MODEL;
      ...
      if (prev === undefined) delete process.env.OPENHOMIE_WHISPER_MODEL;
      else process.env.OPENHOMIE_WHISPER_MODEL = prev;
    }
  });
  ...
  test('kills subprocess and cleans temp dir on abort', async () => {
    ...
    const prevModel = process.env.OPENHOMIE_WHISPER_MODEL;
    const prevCli = process.env.OPENHOMIE_WHISPER_CLI;
    ...
    process.env.OPENHOMIE_WHISPER_MODEL = '/dev/null';
    process.env.OPENHOMIE_WHISPER_CLI = cliPath;
    ...
    try {
      ...
      clearTimeout(t);
      if (prevModel === undefined) delete process.env.OPENHOMIE_WHISPER_MODEL;
      else process.env.OPENHOMIE_WHISPER_MODEL = prevModel;
      if (prevCli === undefined) delete process.env.OPENHOMIE_WHISPER_CLI;
      else process.env.OPENHOMIE_WHISPER_CLI = prevCli;
