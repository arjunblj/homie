...
import { loadOpenhomieConfig } from './load.js';

describe('loadOpenhomieConfig (more)', () => {
  test('throws when homie.toml not found', async () => {
    const tmp = await mkdtemp(path.join(os.tmpdir(), 'homie-noconfig-'));
    try {
      await expect(loadOpenhomieConfig({ cwd: tmp, env: {} })).rejects.toThrow(
  ...
  test('throws when min_delay_ms > max_delay_ms', async () => {
    ...
    try {
      ...
      await expect(loadOpenhomieConfig({ cwd: tmp, env: {} })).rejects.toThrow('min_delay_ms');
    } finally {
      await rm(tmp, { recursive: true, force: true });
    }
  });
  ...
  test('rejects absolute paths outside the project directory', async () => {
    ...
    try {
      ...
      await expect(loadOpenhomieConfig({ cwd: tmp, env: {} })).rejects.toThrow('paths.data_dir');
    } finally {
      await rm(tmp, { recursive: true, force: true });
    }
  });
  ...
  test('treats blank model env vars as unset (fallbacks remain non-empty)', async () => {
    ...
    try {
      ...
      const { config } = await loadOpenhomieConfig({
        cwd: tmp,
        env: {
          OPENHOMIE_MODEL_DEFAULT: '   ',
          OPENHOMIE_MODEL_FAST: ' \n ',
          OPENHOMIE_TIMEZONE: 'UTC',
        },
      });
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_ENGINE_LIMITER_CAPACITY: '-1',
            OPENHOMIE_TIMEZONE: 'UTC',
          },
        }),
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_ENGINE_CONTEXT_MAX_TOKENS_DEFAULT: '999999999',
            OPENHOMIE_TIMEZONE: 'UTC',
          },
        }),
  ...
  test('resolves provider aliases and parses falsey sleep env values', async () => {
    ...
    try {
      ...
      const { config } = await loadOpenhomieConfig({
        cwd: tmp,
        env: {
          OPENHOMIE_MODEL_PROVIDER: 'openrouter',
          OPENHOMIE_SLEEP_MODE: '0',
          OPENHOMIE_TIMEZONE: 'UTC',
        },
      });
  ...
  test('resolves openai provider alias to OpenAI base URL', async () => {
    ...
    try {
      ...
      const { config } = await loadOpenhomieConfig({
        cwd: tmp,
        env: { OPENHOMIE_MODEL_PROVIDER: 'openai', OPENHOMIE_TIMEZONE: 'UTC' },
      });
  ...
  test('resolves mpp provider alias with default base URL', async () => {
    ...
    try {
      ...
      const { config } = await loadOpenhomieConfig({
        cwd: tmp,
        env: { OPENHOMIE_MODEL_PROVIDER: 'mpp', OPENHOMIE_TIMEZONE: 'UTC' },
      });
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_MODEL_PROVIDER: 'openai-compatible',
            OPENHOMIE_MODEL_BASE_URL: 'file:///tmp/secret',
          },
        }),
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_MODEL_PROVIDER: 'mpp',
            OPENHOMIE_MODEL_BASE_URL: 'not-a-url',
          },
        }),
      ).rejects.toThrow('Invalid model.base_url');
    } finally {
  ...
  test('resolves claude_code and codex aliases', async () => {
    ...
    try {
      ...
      const claude = await loadOpenhomieConfig({
        cwd: tmp,
        env: { OPENHOMIE_MODEL_PROVIDER: 'claude_code', OPENHOMIE_TIMEZONE: 'UTC' },
      });
      expect(claude.config.model.provider.kind).toBe('claude-code');

      const codex = await loadOpenhomieConfig({
        cwd: tmp,
        env: { OPENHOMIE_MODEL_PROVIDER: 'codex', OPENHOMIE_TIMEZONE: 'UTC' },
      });
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_MODEL_PROVIDER: 'weird',
            OPENHOMIE_MODEL_BASE_URL: 'http://example.test/v1',
          },
        }),
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_MODEL_DEFAULT: 'bad model',
            OPENHOMIE_MODEL_FAST: 'fast-model',
          },
        }),
  ...
  test('csv env parser ignores empty entries', async () => {
    ...
    try {
      ...
      const { config } = await loadOpenhomieConfig({
        cwd: tmp,
        env: {
          OPENHOMIE_TOOLS_RESTRICTED_ALLOWLIST: ',,,shell_exec,,',
        },
      });
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_SLEEP_MODE: 'maybe',
          },
        }),
      ).rejects.toThrow('Invalid OPENHOMIE_SLEEP_MODE');
    } finally {
      await rm(tmp, { recursive: true, force: true });
    }
  });
  ...
  test('parses quoted csv env lists for tools allowlists', async () => {
    ...
    try {
      ...
      const { config } = await loadOpenhomieConfig({
        ...
        env: {
          OPENHOMIE_TIMEZONE: 'UTC',
          OPENHOMIE_TOOLS_DANGEROUS_ALLOWLIST: '"shell_exec", "fs_write", shell_exec',
        },
      });
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_TOOLS_RESTRICTED_ALLOWLIST: 'shell_exec,bad-name',
          },
        }),
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_TOOLS_DANGEROUS_ALLOW_ALL: 'true',
            OPENHOMIE_TOOLS_DANGEROUS_ALLOWLIST: 'shell_exec',
          },
        }),
    ...
    try {
      ...
      await expect(
        loadOpenhomieConfig({
          cwd: tmp,
          env: {
            OPENHOMIE_TOOLS_RESTRICTED_ALLOWLIST: '"shell_exec,fs_write',
          },
        }),
