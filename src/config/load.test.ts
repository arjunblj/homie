...
`);
    ...
    const { config } = await loadOpenhomieConfig({
      ...
      configPath,
      env: {
        OPENHOMIE_MODEL_DEFAULT: 'claude-env',
        OPENHOMIE_MODEL_FAST: 'claude-fast-env',
        OPENHOMIE_TOOLS_DANGEROUS_ENABLED_FOR_OPERATOR: 'true',
        OPENHOMIE_TOOLS_DANGEROUS_ALLOWLIST: 'shell_exec,fs_write',
