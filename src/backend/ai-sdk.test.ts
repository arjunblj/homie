...
const baseConfig = (overrides: Partial<OpenhomieConfig['model']>): OpenhomieConfig => ({
  schemaVersion: 1,
  model: {
...
describe('AiSdkBackend', () => {
  ...
  test('throws if anthropic provider missing key', async () => {
    const cfg: OpenhomieConfig = baseConfig({ provider: { kind: 'anthropic' } });
  ...
  test('throws for cli-session providers (use backend factory)', async () => {
    const claudeCfg: OpenhomieConfig = baseConfig({ provider: { kind: 'claude-code' } });
    await expect(AiSdkBackend.create({ config: claudeCfg, env: {} })).rejects.toThrow(
      'requires the CLI backend factory',
    );

    const codexCfg: OpenhomieConfig = baseConfig({ provider: { kind: 'codex-cli' } });
